...
func TestMatrixComparison(t *testing.T) {
	...
	m3 := matrix.NewMatrix([][]float64{
		{1, 2, 3, 4},
		{5.5, 6.5, 7.5, 8.5},
		{9, 10, 11, 12},
		{13.5, 14.5, 15.5, 16.51},
	})
	m4 := matrix.NewMatrix([][]float64{
		{1, 2, 3, 4},
		{5.5, 6.5, 7.5, 8.5},
		{9, 10, 11, 12},
...
func TestMatrixMultiplicationByTuple(t *testing.T) {
	...
	a := tuple.NewTuple(1, 2, 3, 1)

	// Then
	assert.EqualValues(t, tuple.NewTuple(18, 24, 33, 1), m.MultiplyTuple(a))
}

func TestMatrixIdentityMultiplication(t *testing.T) {
	// Given
	i := matrix.IdentityMatrix(4)
	m := matrix.NewMatrix([][]float64{
		{0, 1, 2, 4},
		{1, 2, 4, 8},
		{2, 4, 8, 16},
...
func TestMoreInversions(t *testing.T) {
	...
	// Given
	C := matrix.NewMatrix([][]float64{
		{9, 3, 0, 9},
		{-5, -2, -6, -3},
		{-4, 9, 6, 4},
		{-7, 6, 6, 2},
	})
	D := matrix.NewMatrix([][]float64{
...
func TestMultiplyByTransformationMatrix(t *testing.T) {
	// Given
	trans := matrix.IdentityMatrix(4).Translate(5, -3, 2)
	p := tuple.Point(-3, 4, 5)
...
func TestScalingMatrixAppliedToPoint(t *testing.T) {
	// Given
	scale := matrix.IdentityMatrix(4).Scale(2, 3, 4)
	p := tuple.Point(-4, 6, 8)
...
func TestReflectionIsScalingByANegativeValue(t *testing.T) {
	// Given
	scale := matrix.IdentityMatrix(4).Scale(-1, 1, 1)
...
func TestRotatingAPointAroundTheXAxis(t *testing.T) {
	// Given
	p := tuple.Point(0, 1, 0)
	halfQuarter := matrix.IdentityMatrix(4).RotateX(math.Pi / 4)
	fullQuarter := matrix.IdentityMatrix(4).RotateX(math.Pi / 2)
...
func TestRotationAroundTheYAxis(t *testing.T) {
	// Given
	p := tuple.Point(0, 0, 1)
	halfQuarter := matrix.IdentityMatrix(4).RotateY(math.Pi / 4)
	fullQuarter := matrix.IdentityMatrix(4).RotateY(math.Pi / 2)
...
func TestRotationAroundTheZAxis(t *testing.T) {
	// Given
	p := tuple.Point(0, 1, 0)
	halfQuarter := matrix.IdentityMatrix(4).RotateZ(math.Pi / 4)
	fullQuarter := matrix.IdentityMatrix(4).RotateZ(math.Pi / 2)
...
func TestShearingXInProportionToY(t *testing.T) {
	// Give
	sh := matrix.IdentityMatrix(4).Shear(1, 0, 0, 0, 0, 0)
	p := tuple.Point(2, 3, 4)
...
func TestShearingXInProportionToZ(t *testing.T) {
	// Give
	sh := matrix.IdentityMatrix(4).Shear(0, 1, 0, 0, 0, 0)
	p := tuple.Point(2, 3, 4)
...
func TestShearingYInProportionToX(t *testing.T) {
	// Give
	sh := matrix.IdentityMatrix(4).Shear(0, 0, 1, 0, 0, 0)
	p := tuple.Point(2, 3, 4)
...
func TestShearingYInProportionToZ(t *testing.T) {
	// Give
	sh := matrix.IdentityMatrix(4).Shear(0, 0, 0, 1, 0, 0)
	p := tuple.Point(2, 3, 4)
...
func TestShearingZInProportionToX(t *testing.T) {
	// Give
	sh := matrix.IdentityMatrix(4).Shear(0, 0, 0, 0, 1, 0)
	p := tuple.Point(2, 3, 4)
...
func TestShearingZInProportionToY(t *testing.T) {
	// Give
	sh := matrix.IdentityMatrix(4).Shear(0, 0, 0, 0, 0, 1)
	p := tuple.Point(2, 3, 4)
...
func TestTransformationsAreAppliedInSequence(t *testing.T) {
	...
	A := matrix.IdentityMatrix(4).RotateX(math.Pi / 2)
	B := matrix.IdentityMatrix(4).Scale(5, 5, 5)
	C := matrix.IdentityMatrix(4).Translate(10, 5, 7)
...
func TestTransformationFluentAPI(t *testing.T) {
	...
	// When
	tp := matrix.IdentityMatrix(4).RotateX(math.Pi/2).Scale(5, 5, 5).Translate(10, 5, 7).Transform(p)
...
func TestTheTransformationMatrixForTheDefaultOrientation(t *testing.T) {
	...
	// Then
	assert.True(t, matrix.IdentityMatrix(4).Equals(T))
}
...
func TestViewTransformationMatrixLookingInThePositiveZDirection(t *testing.T) {
	...
	// Then
	assert.True(t, matrix.ScalingMatrix(-1, 1, -1).Equals(T))
}
...
func TestTheViewTransformationMovesTheWorldNotTheEye(t *testing.T) {
	...
	// Then
	assert.True(t, matrix.TranslationMatrix(0, 0, -8).Equals(T))
}
