...
func NewCanvas(w, h int) *Canvas {
	pixels := make([][]Colour, w)

	for i := 0; i < w; i++ {
		pixels[i] = make([]Colour, h)
		for j := 0; j < h; j++ {
...
func (c *Canvas) WritePixel(x, y int, colour Colour) {
	c.Pixels[x][y] = colour
}

func (c *Canvas) PixelAt(x, y int) Colour {
	return c.Pixels[x][y]
}

// TODO: Refactor
func (c *Canvas) CanvasToPPM() PPM {
